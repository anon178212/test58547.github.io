var jsPsychSurveyLikert = (function (jspsych) {
  ...
  class SurveyLikertPlugin {
    ...
    trial(display_element, trial) {
      ...
      var html = "";
//edit the following section to optimize user experience (e.g., width, font, font-size, etc.)
html += '<style id="jspsych-survey-likert-css">';

/* Container: vertical layout (prompt above, scale below) */
html += ".jspsych-survey-likert-question { margin-bottom: 30px; }";

/* Prompt: optional max-width, centered */
html += ".jspsych-survey-likert-statement {display: block;font-size: 20px;max-width: 600px;width: auto;margin: 0 auto 10px auto;text-align: center;word-wrap: break-word;}";

/* Likert scale block: fixed width, centered */
html += ".jspsych-survey-likert-opts { list-style: none; width: 700px; margin: 0 auto; padding: 0; font-size: 20px; display: flex; justify-content: space-between; position: relative; }";

/* Optional: line under scale */
html += ".jspsych-survey-likert-opts:before { content: ''; position: absolute; top: 11px; left: 0; right: 0; background-color:#efefef; height: 4px; z-index: -1; }";

/* Each option equally spaced */
html += ".jspsych-survey-likert-opts li { display: inline-block; text-align: center; flex: 1; }";

/* Bigger radio buttons */
html += ".jspsych-survey-likert-opts li input[type=radio] { display: block; margin: auto; width: 20px; height: 20px; }";

html += ".jspsych-survey-likert-opt-label { \
...
html += "</style>";
      if (trial.preamble !== null) {
		//preamble takes the full page of width
        //html += '<div id="jspsych-survey-likert-preamble" class="jspsych-survey-likert-preamble">' + trial.preamble + "</div>";
		//preamble takes 900px of width
		html += '<style>#jspsych-survey-likert-preamble{max-width:900px;margin:0 auto;}</style><div id="jspsych-survey-likert-preamble" class="jspsych-survey-likert-preamble">' + trial.preamble + "</div>";
      }
      ...
      for (var i = 0; i < trial.questions.length; i++) {
        ...
        html += '<div class="jspsych-survey-likert-question">';
